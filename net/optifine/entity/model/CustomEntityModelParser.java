...
public class CustomEntityModelParser {
  ...
  public static CustomEntityRenderer parseEntityRender(JsonObject obj, String path) {
    ...
    CustomModelRenderer[] acustommodelrenderer = list.<CustomModelRenderer>toArray(new CustomModelRenderer[list.size()]);
  ...
  private static void copyJsonElements(JsonObject objFrom, JsonObject objTo) {
    for (Map.Entry<String, JsonElement> entry : (Iterable<Map.Entry<String, JsonElement>>)objFrom.entrySet()) {
  ...
  public static CustomModelRenderer parseCustomModelRenderer(JsonObject elem, int[] textureSize, String basePath) {
    ...
    if (jsonarray != null) {
      ...
      if (list.size() > 0) {
        ModelVariableUpdater[] amodelvariableupdater = list.<ModelVariableUpdater>toArray(new ModelVariableUpdater[list.size()]);
