...
public enum FunctionType {
  ...
  public float evalFloat(IExpression[] args) {
    ...
    switch (this) {
      ...
      case TIME:
        ...
        if (worldClient == null)
          return 0.0F; 
        return (float)(worldClient.getTotalWorldTime() % 24000L) + Config.renderPartialTicks;
      case IF:
        i = (args.length - 1) / 2;
        for (k = 0; k < i; k++) {
          int l = k * 2;
      ...
      case SMOOTH:
        j = (int)evalFloat(args, 0);
        f4 = evalFloat(args, 1);
        f5 = (args.length > 2) ? evalFloat(args, 2) : 1.0F;
        f6 = (args.length > 3) ? evalFloat(args, 3) : f5;
        f7 = Smoother.getSmoothValue(j, f4, f5, f6);
        return f7;
    } 
  ...
  public boolean evalBool(IExpression[] args) {
    ...
    switch (this) {
      ...
      case EQUALS:
        f1 = evalFloat(args, 0) - evalFloat(args, 1);
        f2 = evalFloat(args, 2);
