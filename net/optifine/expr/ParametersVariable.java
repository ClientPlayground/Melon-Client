...
public class ParametersVariable implements IParameters {
  ...
  public ExpressionType[] getParameterTypes(IExpression[] arguments) {
    ...
    int k = 0;
    int l;
    for (l = 0; l + this.repeat.length <= j && i + l + this.repeat.length <= this.maxCount; l += this.repeat.length)
      k++; 
    ...
    ExpressionType[] aexpressiontype = list.<ExpressionType>toArray(new ExpressionType[list.size()]);
