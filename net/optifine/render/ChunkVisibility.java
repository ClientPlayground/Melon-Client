...
public class ChunkVisibility {
    ...
    for (int i2 = l; i2 < i1; i2++) {
      for (int j2 = j1; j2 < k1; j2++) {
        ...
        if (!chunk1.isEmpty()) {
          ...
          ClassInheritanceMultiMap[] arrayOfClassInheritanceMultiMap = chunk1.getEntityLists();
          for (int i3 = arrayOfClassInheritanceMultiMap.length - 1; i3 > l1; i3--) {
            ClassInheritanceMultiMap<Entity> classinheritancemultimap1 = arrayOfClassInheritanceMultiMap[i3];
  ...
  private static EnumFacing[][] makeEnumFacingArrays(boolean opposite) {
    ...
    for (int j = 0; j < i; j++) {
      ...
      EnumFacing[] aenumfacing1 = list.<EnumFacing>toArray(new EnumFacing[list.size()]);
  ...
  public static EnumFacing[] getFacingsNotOpposite(int setDisabled) {
    int i = (setDisabled ^ 0xFFFFFFFF) & 0x3F;
