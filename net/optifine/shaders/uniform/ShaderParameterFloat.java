  ...
  public float eval(int index1, int index2) {
    if (this.indexNames1 == null || (index1 >= 0 && index1 <= this.indexNames1.length)) {
      if (this.indexNames2 == null || (index2 >= 0 && index2 <= this.indexNames2.length)) {
        ...
        switch (this) {
          case BIOME:
            blockpos2 = Shaders.getCameraPosition();
            biomegenbase2 = Shaders.getCurrentWorld().getBiomeGenForCoords(blockpos2);
            return biomegenbase2.biomeID;
          case TEMPERATURE:
            blockpos1 = Shaders.getCameraPosition();
            biomegenbase1 = Shaders.getCurrentWorld().getBiomeGenForCoords(blockpos1);
            return (biomegenbase1 != null) ? biomegenbase1.getFloatTemperature(blockpos1) : 0.0F;
          case RAINFALL:
            pos = Shaders.getCameraPosition();
            biome = Shaders.getCurrentWorld().getBiomeGenForCoords(pos);
