...
public final class SpawnerAnimals {
  ...
  private Map<Class, EntityLiving> mapSampleEntitiesByClass = (Map)new HashMap<>();
    ...
    for (EnumCreatureType enumcreaturetype : EnumCreatureType.values()) {
      if ((!enumcreaturetype.getPeacefulCreature() || spawnPeacefulMobs) && (enumcreaturetype.getPeacefulCreature() || spawnHostileMobs) && (!enumcreaturetype.getAnimal() || p_77192_4_)) {
        ...
        if (k4 <= l4) {
          ...
          label118: for (ChunkCoordIntPair chunkcoordintpair1 : collection) {
            BlockPosM blockPosM = getRandomChunkPosition(worldServerIn, chunkcoordintpair1.chunkXPos, chunkcoordintpair1.chunkZPos, blockposm);
            int k1 = blockPosM.getX();
            int l1 = blockPosM.getY();
            int i2 = blockPosM.getZ();
            Block block = worldServerIn.getBlockState((BlockPos)blockPosM).getBlock();
                ...
                for (int l3 = 0; l3 < 4; l3++) {
                  ...
                  if (!worldServerIn.isAnyPlayerWithinRangeAt(f, i3, f1, 24.0D) && blockpos2.distanceSq(f, i3, f1) >= 576.0D) {
                    ...
                    if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biomegenbase$spawnlistentry, blockpos1) && canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biomegenbase$spawnlistentry.entityClass), worldServerIn, blockpos1)) {
                      ...
                      boolean flag2 = Reflector.ForgeEventFactory_canEntitySpawn.exists() ? ReflectorForge.canEntitySpawn(entityliving, worldServerIn, f, i3, f1) : ((entityliving.getCanSpawnHere() && entityliving.isNotColliding()));
                      if (flag2) {
          ...
          for (int k1 = 0; !flag && k1 < 4; k1++) {
            ...
            if (canCreatureTypeSpawnAtLocation(EntityLiving.SpawnPlacementType.ON_GROUND, worldIn, blockpos)) {
              ...
              if (Reflector.ForgeEventFactory_canEntitySpawn.exists()) {
                Object object = Reflector.call(Reflector.ForgeEventFactory_canEntitySpawn, new Object[] { entityliving, worldIn, Float.valueOf(j + 0.5F), Integer.valueOf(blockpos.getY()), Float.valueOf(k + 0.5F) });
