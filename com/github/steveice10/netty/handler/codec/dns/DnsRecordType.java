...
public class DnsRecordType implements Comparable<DnsRecordType> {
  ...
  static {
    DnsRecordType[] all = { 
        A, NS, CNAME, SOA, PTR, MX, TXT, RP, AFSDB, SIG, 
        KEY, AAAA, LOC, SRV, NAPTR, KX, CERT, DNAME, OPT, APL, 
        DS, SSHFP, IPSECKEY, RRSIG, NSEC, DNSKEY, DHCID, NSEC3, NSEC3PARAM, TLSA, 
        HIP, SPF, TKEY, TSIG, IXFR, AXFR, ANY, CAA, TA, DLV };
  ...
  public String toString() {
    ...
    if (text == null)
      this.text = text = this.name + '(' + intValue() + ')'; 
    return text;
  }
}
