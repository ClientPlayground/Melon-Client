...
public class EntityIdRewriter {
  ...
  public static void toClient(CompoundTag tag) {
    if (tag.get("id") instanceof StringTag) {
      StringTag id = (StringTag)tag.get("id");
      String newName = (String)oldToNewNames.get(id.getValue());
      if (newName != null)
        id.setValue(newName); 
    } 
  }
  ...
  public static void toServerItem(Item item) {
    if (hasEntityTag(item)) {
      ...
      if (entityTag.get("id") instanceof StringTag) {
        ...
        String newName = (String)oldToNewNames.inverse().get(id.getValue());
        if (newName != null)
          id.setValue(newName); 
      } 
    } 
  ...
  private static boolean hasEntityTag(Item item) {
    if (item != null && item.getIdentifier() == 383) {
      ...
      if (tag != null && tag.contains("EntityTag") && tag.get("EntityTag") instanceof CompoundTag && (
        (CompoundTag)tag.get("EntityTag")).get("id") instanceof StringTag)
        return true; 
    } 
    return false;
  }
